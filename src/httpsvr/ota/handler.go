...
func (s *DefaultOTAService) handlePostOTA(c *gin.Context) {
	...
	if deviceID == "" {
		utils.Custom(c, http.StatusBadRequest, ErrorResponse{Success: false, Message: "缺少 device-id"})
		return
	}

	var raw map[string]interface{}
	if err := c.ShouldBindJSON(&raw); err != nil {
		utils.Custom(c, http.StatusBadRequest, gin.H{"success": false, "message": "解析失败: " + err.Error()})
...
func (s *DefaultOTAService) HandleFirmwareDownload() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if reqPath == "" {
			utils.Custom(c, http.StatusBadRequest, gin.H{"success": false, "message": "invalid file path"})
			return
		}
		...
		if err != nil || fi.IsDir() {
			utils.Custom(c, http.StatusNotFound, gin.H{"success": false, "message": "file not found"})
			return
		}
