...
func MP3ToPCMData(audioFile string) ([][]byte, error) {
	...
	frameBytes := mp3SampleRate * 2 * pcmFrameDurationMs / 1000
	if frameBytes <= 0 {
		frameBytes = len(monoPcmDataBytes)
	}

	frames := chunkPCMBytes(monoPcmDataBytes, frameBytes)
	return frames, nil
}
...
func chunkPCMBytes(data []byte, frameBytes int) [][]byte {
	if frameBytes <= 0 {
		return [][]byte{data}
	}
	chunks := make([][]byte, 0, (len(data)+frameBytes-1)/frameBytes)
	...
	if len(chunks) == 0 {
		chunks = append(chunks, make([]byte, frameBytes))
	}
	return chunks
}
